...
AddEventHandler('jsfour-licenseplate', function()
  ...
  if distance ~= -1 and distance <= 3.0 then -- verifica si hay veh
  local oldplate = GetVehicleNumberPlateText(vehicle)

    ESX.TriggerServerCallback('jsfour-licenseplate:update', function( cb )        
    if cb == 'found' then

      local newPlatee = messages[setLocale]['plateText']
                SetVehicleNumberPlateText(vehicle, (newPlatee, math.random( 100, 300 )))
                plate = nil
      elseif cb == 'error' then
                ESX.ShowNotification(messages[setLocale]['error'])
    end
